...
I have hidden the code in the final document for easier viewing, but please do see the original .Rmd or .md for the code producing this report.
...
For some purposes, zip code is too small of a geographic unit to communicate clear trends. We can aggregate zip codes into counties. Before looking at county totals by party however, we should first observe the disparity in total contributions. If we fill the county map by total contributions, we can see that the majority of contributions come from a few counties surrounding the two major metropolitan areas, Philadelphia and Pittsburgh. Mouse over the county for exact figures.
...
This of course though should not be too surprising as it closely mirrors the population that the map represents. We could get county population data from a number of places, but I have chosen to scrape the latest census data from pennsylvania-demographics.com. The original population data can be found [here](https://www.pennsylvania-demographics.com/counties_by_population).
